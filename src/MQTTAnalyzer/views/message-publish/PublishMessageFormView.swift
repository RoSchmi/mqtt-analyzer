...
struct PublishMessageFormModalView: View {
	...
	var body: some View {
		NavigationView {
			PublishMessageFormView( function_on_PublishMessageFormModalView: {self.doSomething(titleParameter: $0, topic: $1)}, message: self.model, fixedMessagesModel: fixedMessagesModel, type:
				self.$model.messageType)
				.font(.caption)
	...
	func doSomething(titleParameter: String, topic: String) {print(titleParameter); print(topic) }
...
struct PublishMessageFormView: View {
	...
	var function_on_PublishMessageFormModalView: (String, String) -> Void
	
	@ObservedObject var message: PublishMessageFormModel
	
	//RoSchmi
	//@ObservedObject var model: MessageModel
	//@ObservedObject var fixedMessage: PublishFixedMessagesModel
	...
	//RoSchmi
	//@ObservedObject var theFixedMessage: PublishFixedMessagesModel
	@Binding var type: PublishMessageType
	var body: some View {
		Form {
			// RoSchmi
			NavigationLink(destination: PublishFixedMessagesView( function_on_PublishMessageFormView: {self.handOverToParent(titleParameter: $0, topic: $1)}, message:
	...
	func handOverToParent(titleParameter: String, topic: String){message.topic = topic; function_on_PublishMessageFormModalView(titleParameter, topic)}
